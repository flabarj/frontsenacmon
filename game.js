// ===== Caminhos dos assets (GitHub Pages) =====
...
const Game = {
  players: [
    ...
    { id:'P2', berries:100, pos:0, starter:null, captured:0, elmColor:'#ff9fe0', isCPU:true  }, // P2 vira CPU se existir
  ],
  nPlayers: 1,
  turn: 0,
  round: 1,
...
function seedZones(){
  ...
  // 8 batalhas, 2 capturas, 6 bônus, 6 perdas -> 8 neutras
...
function endOrNext(){
  if(Game.round>=Game.roundLimit && Game.turn===(Game.nPlayers-1)){
    ...
    setMsg(msg); logItem(msg,'warn'); return;
  }
  nextTurn();
}

function rollDice(){
  if(isCPUTurn()) BTN_ROLL().disabled = true;
